            ...
            <div class="col-md-12">
                ...
                <div class="card shadow">
                    ...
                    <div class="card-header bg-custom-gradient text-white text-center">
                        <h5 class="mb-0 fw-bold">Reporte Inventario (Stock)</h5>
                    </div>

                    <div class="card-body">
                        {{-- Formulario de selección de tipo --}}
                        <form action="{{ route('reports.stock') }}" method="GET" class="mb-4">
                            <div class="row align-items-end g-3">
                                <div class="col-md-6">
                                    <label for="report_type" class="form-label fw-bold">Tipo de Reporte</label>
                                    <select name="report_type" id="report_type" class="form-select" required>
                                        <option value="">Seleccione una opción...</option>
                                        <option value="available_by_equipment"
                                            {{ $reportType == 'available_by_equipment' ? 'selected' : '' }}>
                                            Equipamiento disponible (ordenado por equipo)
                                        </option>
                                        <option value="available_by_store"
                                            {{ $reportType == 'available_by_store' ? 'selected' : '' }}>
                                            Equipamiento disponible (Ordenado por Almacén)
                                        </option>
                                        <option value="unavailable_by_equipment"
                                            {{ $reportType == 'unavailable_by_equipment' ? 'selected' : '' }}>
                                            Equipamiento no disponible (Ordenado por equipo)
                                        </option>
                                    </select>
                                </div>
                        ...
                        @if ($data->isNotEmpty())
                                        ...
                                        <tbody>
                                            @foreach ($data as $item)
                                                <tr>
                                                    ...
                                                    <td
                                                        class="text-center fw-bold {{ $item->stock <= $item->umbral ? 'text-danger' : 'text-success' }}">
                                                        {{ $item->stock }}
                                                    </td>
                            ...
                            <div class="d-flex justify-content-end gap-2 mt-3">
                                <form action="{{ route('reports.stock.pdf') }}" method="GET" target="_blank"
                                    class="d-inline">
                                ...
                                <form action="{{ route('reports.stock.excel') }}" method="GET" class="d-inline">
                                    <input type="hidden" name="report_type" value="{{ $reportType }}">
                        ...
                        @elseif(request()->has('generate'))
                            <div class="alert alert-info text-center">
                                ...
                                No se encontraron registros para el tipo de reporte seleccionado.
                            </div>
                        @endif
...
@push('styles')
    <style>
        /* Contenedor para scroll vertical + horizontal */
        .table-wrapper {
        ...
        /* Asegurar que las celdas no se rompan feo */
        .table-report td,
        .table-report th {
            white-space: nowrap;
